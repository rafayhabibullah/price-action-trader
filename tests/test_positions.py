...
def test_short_sl_hit():
    """Short trade: SL above entry. High[3] hits SL."""
    data = {
        "open":   [100, 101, 102, 103, 104, 100],
        "high":   [101, 102, 103, 106, 105, 101],  # high[3]=106 >= SL=105
        "low":    [99,  100, 101, 102, 103, 99],
        "close":  [101, 101, 102, 105, 103, 100],
        "volume": [1000.0] * 6,
        "signal": [0, 0, -1, 0, 0, 0],
        "sl":     [np.nan, np.nan, 105.0, np.nan, np.nan, np.nan],
        "tp":     [np.nan, np.nan, 95.0, np.nan, np.nan, np.nan],
    }
    df = pd.DataFrame(data, index=pd.date_range("2024-01-01", periods=6, freq="1h"))
    trades = simulate_trades(df, starting_capital=1000.0, risk_per_trade=0.02)
    assert len(trades) == 1
...
def test_timeout_exit():
    """Trade should exit after max_bars regardless of SL/TP."""
    n = 15
    data = {
        "open":   [100.0] * n,
        "high":   [101.0] * n,
        ...
        "signal": [1] + [0] * (n - 1),
        "sl":     [95.0] + [np.nan] * (n - 1),
        "tp":     [200.0] + [np.nan] * (n - 1),  # TP far away, won't be hit
    }
    df = pd.DataFrame(data, index=pd.date_range("2024-01-01", periods=n, freq="1h"))
    trades = simulate_trades(df, starting_capital=1000.0, risk_per_trade=0.02, max_bars=5)
    assert len(trades) == 1
